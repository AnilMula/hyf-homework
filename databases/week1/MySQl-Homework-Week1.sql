...
select count(*) as "invalid-due-date-tasks" from task where due_date is null;
-- OUTPUT: 8
...
select title, description, due_date from task where title like '%database%' || description like '%database%';
-- OUTPUT: 
/* 
	title	description	due_date
	Setup salary databases for accounting	Use MySQL	2017-12-05 00:19:08
	Learn how databases work		2017-12-18 05:08:05
	Make the databases perform better	It should be possible to optimize the indexes	2017-12-01 13:28:35
	Backup databases to external disk	Remember to store the disk in another physical location	2017-12-23 14:21:01
	Learn about NoSQL databases	MongoDB, CouchDB, etc.	2017-12-23 10:13:42
*/
...
select status.name, count(task.id) as "status-count" from task join status on task.status_id=status.id group by status.id;
-- OUTPUT: 
/*
	name	status-count
	Not started	8
